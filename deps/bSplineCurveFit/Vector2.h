...
template <typename Real> using Vector2 = Vector<2, Real>;
...
// orthonormal set.
template <typename Real>
Real ComputeOrthogonalComplement(int32_t numInputs, Vector2<Real> *v,
                                 bool robust = false)
{
    if (numInputs == 1)
    {
                         ...
                         Real epsilon = static_cast<Real>(0))
{
    ...
    array<Vector2<Real>, 3> diff = {v0 - v2, v1 - v2, p - v2};
...
template <typename Real> class IntrinsicsVector2
{
public:
    // The constructor sets the class members based on the input set.
    IntrinsicsVector2(int32_t numVectors, Vector2<Real> const *v,
                      Real inEpsilon)
        : epsilon(inEpsilon), dimension(0),
          maxRange((Real)0), origin{(Real)0, (Real)0}, extremeCCW(false)
    {
        min[0] = (Real)0;
        min[1] = (Real)0;
        max[0] = (Real)0;
        ...
        if (numVectors > 0 && v && epsilon >= (Real)0)
        {
            ...
            // current min and max.
            int32_t j{};
            array<int32_t, 2> indexMin{}, indexMax{};
            for (j = 0; j < 2; ++j)
            {
                min[j] = v[0][j];
                max[j] = min[j];
