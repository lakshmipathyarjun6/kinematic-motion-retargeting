...
template <int32_t N, typename Real> class Vector
{
public:
    ...
    Vector(array<Real, N> const &values) : mTuple(values) {}
    ...
    // constructor!
    Vector(initializer_list<Real> values)
    {
        int32_t const numValues = static_cast<int32_t>(values.size());
        if (N == numValues)
        {
            copy(values.begin(), values.end(), mTuple.begin());
        }
        else if (N > numValues)
        {
            copy(values.begin(), values.end(), mTuple.begin());
            fill(mTuple.begin() + numValues, mTuple.end(), (Real)0);
        }
        else // N < numValues
        {
            copy(values.begin(), values.begin() + N, mTuple.begin());
        }
    }
    ...
    void MakeZero() { fill(mTuple.begin(), mTuple.end(), (Real)0); }

    // All components are 1.
    void MakeOnes() { fill(mTuple.begin(), mTuple.end(), (Real)1); }

    // Component d is 1, all others are zero.
    void MakeUnit(int32_t d)
    {
        fill(mTuple.begin(), mTuple.end(), (Real)0);
        if (0 <= d && d < N)
        {
            mTuple[d] = (Real)1;
        }
