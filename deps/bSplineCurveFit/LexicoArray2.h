...
template <typename Real> class LexicoArray2<true, Real>
{
public:
    LexicoArray2(int32_t numRows, int32_t numCols, Real *matrix)
        : mNumRows(numRows), mNumCols(numCols), mMatrix(matrix)
    {
    }

    inline int32_t GetNumRows() const { return mNumRows; }

    inline int32_t GetNumCols() const { return mNumCols; }
...
template <typename Real> class LexicoArray2<false, Real>
{
public:
    LexicoArray2(int32_t numRows, int32_t numCols, Real *matrix)
        : mNumRows(numRows), mNumCols(numCols), mMatrix(matrix)
...
{
public:
    LexicoArray2(Real *matrix) : mMatrix(matrix) {}

    inline int32_t GetNumRows() const { return NumRows; }

    inline int32_t GetNumCols() const { return NumCols; }
