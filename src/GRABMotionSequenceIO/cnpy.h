...
namespace cnpy
{
                  ...
                  const std::vector<size_t> &shape, std::string mode = "w")
    {
        ...
        if (fp)
        {
            ...
            // global header then read and store the global header. below, we
            // will write the the new data at the start of the global header
    ...
    std::vector<char> create_npy_header(const std::vector<size_t> &shape)
    {
        ...
        if (shape.size() == 1)
            dict += ",";
        dict += "), }";
        // pad with spaces so that preamble+dict is modulo 16 bytes. preamble is
