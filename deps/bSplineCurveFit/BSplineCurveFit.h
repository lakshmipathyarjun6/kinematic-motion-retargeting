...
template <typename Real> class BSplineCurveFit
{
public:
    ...
    BSplineCurveFit(int32_t dimension, int32_t numSamples,
                    Real const *sampleData, int32_t degree, int32_t numControls)
        : mDimension(dimension), mNumSamples(numSamples),
          mSampleData(sampleData), mDegree(degree), mNumControls(numControls),
                       ...
                       static_cast<size_t>(numControls))
    {
        ...
        LogAssert(numControls <= numSamples - degree - 1,
        ...
        memset(ATMat[0], 0,
               static_cast<size_t>(mNumControls) *
                   static_cast<size_t>(mNumSamples) * sizeof(Real));
        for (i0 = 0; i0 < mNumControls; ++i0)
        {
        ...
        fill(mControlData.begin(), mControlData.end(), (Real)0);
        for (i0 = 0; i0 < mNumControls; ++i0)
        {
            ...
            {
                Real const *P =
                    mSampleData + i1 * static_cast<size_t>(mDimension);
        ...
        Real *cEnd1 = &mControlData[static_cast<size_t>(mDimension) *
                                    (static_cast<size_t>(mNumControls) - 1)];
        Real const *sEnd1 =
            &mSampleData[static_cast<size_t>(mDimension) *
                         (static_cast<size_t>(mNumSamples) - 1)];
        for (j = 0; j < mDimension; ++j)
        {
            *cEnd0++ = *sEnd0++;
            *cEnd1++ = *sEnd1++;
    ...
    inline int32_t GetDimension() const { return mDimension; }

    inline int32_t GetNumSamples() const { return mNumSamples; }

    inline Real const *GetSampleData() const { return mSampleData; }
    ...
    inline Real const *GetControlData() const { return &mControlData[0]; }

    inline BasisFunction<Real> const &GetBasis() const { return mBasis; }
