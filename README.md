...
If you are trying to use this codebase with an new articulated hand (e.g. a robot hand stored as a URDF, for example), you will need to create a smooth, coninuous, manifold wrapper in order to use the methods in this codebase. This is because the geometric computations that support the non-isometric shape matching and contact retargeting process fundamentally assume that the domain is fully connected (e.g. it is possible to reach any point on the mesh from any other point on the mesh without running into any disconnected components or non-manifold elements). Unfortunately, articulated hands are comprised of disconnected links.
