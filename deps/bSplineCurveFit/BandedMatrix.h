...
template <typename Real> class BandedMatrix
{
public:
    // Construction and destruction.
    BandedMatrix(int32_t size, int32_t numLBands, int32_t numUBands)
        : mSize(size), mDBand{}, mLBands{}, mUBands{}, mZero((Real)0)
    {
        if (size > 0 && 0 <= numLBands && numLBands < size && 0 <= numUBands &&
            numUBands < size)
        {
            mDBand.resize(size);
            fill(mDBand.begin(), mDBand.end(), (Real)0);
            ...
            if (numLBands > 0)
            {
                ...
                for (auto &band : mLBands)
                {
                    ...
                    fill(band.begin(), band.end(), (Real)0);
                }
            }
    ...
    inline vector<Real> &GetDBand() { return mDBand; }

    inline vector<Real> const &GetDBand() const { return mDBand; }

    inline vector<vector<Real>> &GetLBands() { return mLBands; }

    inline vector<vector<Real>> const &GetLBands() const { return mLBands; }

    inline vector<vector<Real>> &GetUBands() { return mUBands; }

    inline vector<vector<Real>> const &GetUBands() const { return mUBands; }
    ...
    {
        return CholeskyFactor() && SolveLower<RowMajor>(bMatrix, numBColumns) &&
    ...
    // 'bMatrix' must have the storage order specified by the template
    // parameter.
    template <bool RowMajor> bool ComputeInverse(Real *inverse) const
    {
...
private:
    ...
    int32_t mSize;
    vector<Real> mDBand;
    vector<vector<Real>> mLBands, mUBands;
