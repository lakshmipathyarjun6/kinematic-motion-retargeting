...
template <typename Real> struct UniqueKnot
{
    Real t;
    int32_t multiplicity;
};

template <typename Real> struct BasisFunctionInput
{
    // The members are uninitialized.
    BasisFunctionInput()
        : numControls(0), degree(0), uniform(false), periodic(false),
          numUniqueKnots(0), uniqueKnots{}
    {
    }

    // Construct an open uniform curve with t in [0,1].
    BasisFunctionInput(int32_t inNumControls, int32_t inDegree)
        : numControls(inNumControls), degree(inDegree), uniform(true),
          periodic(false), numUniqueKnots(numControls - degree + 1),
    ...
    vector<UniqueKnot<Real>> uniqueKnots;
};

template <typename Real> class BasisFunction
{
public:
    ...
    // the input arrays.
    BasisFunction()
        : mNumControls(0), mDegree(0), mTMin((Real)0), mTMax((Real)0),
          mTLength((Real)0), mOpen(false), mUniform(false), mPeriodic(false)
    {
    }
    ...
    // Support for explicit creation in classes that have array
    ...
    void Create(BasisFunctionInput<Real> const &input)
    {
        ...
        LogAssert(1 <= input.degree && input.degree < input.numControls,
                  "Invalid degree.");
        ...
        size_t numCols =
            static_cast<size_t>(mNumControls) + static_cast<size_t>(mDegree);
    ...
    // Member access.
    inline int32_t GetNumControls() const { return mNumControls; }

    inline int32_t GetDegree() const { return mDegree; }
    ...
    inline Real GetMinDomain() const { return mTMin; }

    inline Real GetMaxDomain() const { return mTMax; }

    inline bool IsOpen() const { return mOpen; }

    inline bool IsUniform() const { return mUniform; }

    inline bool IsPeriodic() const { return mPeriodic; }
    ...
    inline Real const *GetKnots() const { return &mKnots[0]; }
    ...
    void Evaluate(Real t, uint32_t order, int32_t &minIndex,
                  int32_t &maxIndex) const
    {
        ...
        for (j = 1; j <= mDegree; j++)
        {
            d0 = mKnots[static_cast<size_t>(i) + static_cast<size_t>(j)] -
                 mKnots[i];
            d1 = mKnots[static_cast<size_t>(i) + 1] -
                 mKnots[static_cast<size_t>(i) - static_cast<size_t>(j) + 1];
            ...
            if (order >= 1)
            {
                ...
                if (order >= 2)
                {
                    ...
                    mJet[2][j][i] = e0 * invD0;
                    e1 = n1 * mJet[2][j - 1][i - j + 1] -
                         ((Real)2) * mJet[1][j - 1][i - j + 1];
                    ...
                    if (order >= 3)
                    {
                        e0 = n0 * mJet[3][j - 1][i] +
                             ((Real)3) * mJet[2][j - 1][i];
                        mJet[3][j][i] = e0 * invD0;
                        e1 = n1 * mJet[3][j - 1][i - j + 1] -
                             ((Real)3) * mJet[2][j - 1][i - j + 1];
        ...
        for (j = 2; j <= mDegree; ++j)
        {
            ...
            {
                n0 = t - mKnots[k];
                n1 = mKnots[static_cast<size_t>(k) + static_cast<size_t>(j) +
                            1] -
                     t;
                d0 = mKnots[static_cast<size_t>(k) + static_cast<size_t>(j)] -
                     mKnots[k];
                d1 = mKnots[static_cast<size_t>(k) + static_cast<size_t>(j) +
                            1] -
                     mKnots[static_cast<size_t>(k) + 1];
                invD0 = (d0 > (Real)0 ? (Real)1 / d0 : (Real)0);
                invD1 = (d1 > (Real)0 ? (Real)1 / d1 : (Real)0);
                ...
                if (order >= 1)
                {
                    ...
                    if (order >= 2)
                    {
                        e0 = n0 * mJet[2][j - 1][k] +
                             ((Real)2) * mJet[1][j - 1][k];
                        e1 = n1 * mJet[2][j - 1][k + 1] -
                             ((Real)2) * mJet[1][j - 1][k + 1];
                        ...
                        if (order >= 3)
                        {
                            e0 = n0 * mJet[3][j - 1][k] +
                                 ((Real)3) * mJet[2][j - 1][k];
                            e1 = n1 * mJet[3][j - 1][k + 1] -
                                 ((Real)3) * mJet[2][j - 1][k + 1];
...
private:
    ...
    bool mUniform;
    bool mPeriodic;
    vector<UniqueKnot<Real>> mUniqueKnots;
    vector<Real> mKnots;
