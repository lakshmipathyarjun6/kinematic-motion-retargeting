...
This code is intended to be executed as a collection of plugins for Autodesk Maya. If you are unfamiliar Maya, there are a number of YouTube tutorials on getting started. It does not assume any prior knowledge beyond knowing how to navigate the viewport and a basic understanding of the default toolset. The code itself is primarily written in C++, is built with CMake, and requires the C++ 20 standard or higher. There are also some additional MEL (Maya scripting language) scripts that are used for utility purposes and in conjunction with some of the plugins. This codebase is intended to walk users through the process of creating results for *new hands* from scratch or, alternatively, retargeting *new motions* for one of our existing hands.
...
Windows:
```
cd %HOMEPATH%\Documents\maya\2024\
```
...
I strongly recommend starting Maya via command line instead of the Desktop shortuct. This will make debugging / print statements visible in the console. The easiest way to do so is by adding the Maya executable plugin to your path via BashRC. For example, on a Mac running Maya 2024, adding the following line added to your ZProfile will do the trick:
...
## Dealing with Articulated Hands

If you are trying to use this codebase with a new articulated hand (e.g. a robot hand stored as a URDF), you will need to create a smooth, continuous wrapper / character skin for your hand in order to proceed. This is because the geometric computations that support the non-isometric shape matching and contact retargeting process fundamentally assume that the domain is fully connected and manifold (e.g. it is possible to reach any point on the mesh from any other point on the mesh without running into any disconnected components or non-manifold elements). Unfortunately, articulated hands are comprised of disconnected links.

While there are plenty of great tutorials for creating skinned character rigs (e.g. <a href="https://www.youtube.com/watch?v=c538zkwxgTQ">this</a> or <a href="https://www.youtube.com/watch?v=rAZpH3m7K5U&list=PLbvsJz5ZcmxHEPiw_kF3vHjR023rIjR05">this</a>), I do realize there is relatively little online guidance on how to "make a skin" for an articulated entity, or even how to port a URDF into 3D modeling software like Maya or Blender to begin with. In light of that, I am open to making a tutorial video on how I typically do this if there is demand. Please open an Issue on Github or send me an email if you are interested.
